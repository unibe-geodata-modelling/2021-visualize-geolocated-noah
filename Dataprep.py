...
#2.3 current dataframe (from the excel file) has no geometry, so it needs to be converted so it can have a crs which is needed for geopandas to use the input data
...
#2.4.5 As interjectory remark please note that steps 2.5-2.9 must be run as a batch (all lines copy pasted to the python console at once) not one line at a time, or else the program will fail

#2.5 Here the axis are set, in that the figure size is defined
ax = gdf.plot(figsize=(9,9))

#2.6 Here further parameters for the axis are defined, relative to the datapoints so the selected map segment is always appropriate for the datapoints plotted within
ax.set_xlim([gdf.Longitude.min()-0.1,gdf.Longitude.max()+0.1])
ax.set_ylim([gdf.Latitude.min()-0.1,gdf.Latitude.max()+0.1])

#2.7 Here the basemap provider is defined and the previousely defined crs is attributed to it
...
#3.0 While the figure produced can be saved directly (using the save icon at the bottom of the figure) this the line below provides an alternative way to save the figure where the dpi (dots per inch) can be adjusted
